...
#define ERRH_INTERNAL_CALLBACK ((uint32_t) 0)
...
typedef struct
{
    ...
    tVARS_TIMEDATA_STR ti_globalTime;   ///< Actual world time at error event
...
typedef struct
{
    int64_t (*ti_us_readSystemTimeCallback_S64)(void);
    tVARS_TIMEDATA_STR (*ti_readCurrentGlobalTimeCallback_S64)(void);
    void (*reportErrorCallback)(tERRH_ERRORTYPE_E ErrorLvl, uint32_t ModuleId, uint32_t InstanceId, uint32_t ApiId, uint32_t ErrorId);
} tERRH_INITDATA_STR;
