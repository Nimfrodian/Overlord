...
static tB errh_s_moduleInit_tB = false;
...
static int64_t errh_ti_us_fallbackSystemTime_S64(void)
{
    return 0;
}

static tVARS_TIMEDATA_STR errh_ti_us_fallbackGlobalTime_S64(void)
{
    tVARS_TIMEDATA_STR fallbackTimeData = {
        .year_U16 = 0,
        .month_U8 = 0,
        .day_U8 = 0,
        .hour_U8 = 0,
    ...
    return fallbackTimeData;
}

void errh_internalReportError(tERRH_ERRORTYPE_E ErrorLvl, uint32_t ModuleId, uint32_t InstanceId, uint32_t ApiId, uint32_t ErrorId);

static tERRH_INITDATA_STR errh_x_config_str =
{
    .ti_us_readSystemTimeCallback_S64 = errh_ti_us_fallbackSystemTime_S64,
    .ti_readCurrentGlobalTimeCallback_S64 = errh_ti_us_fallbackGlobalTime_S64,
    .reportErrorCallback = errh_internalReportError,
};

void errh_reportError(tERRH_ERRORTYPE_E ErrorLvl, uint32_t ModuleId, uint32_t InstanceId, uint32_t ApiId, uint32_t ErrorId)
{
    errh_x_config_str.reportErrorCallback(ErrorLvl, ModuleId, InstanceId, ApiId, ErrorId);
}
...
void errh_internalReportError(tERRH_ERRORTYPE_E ErrorLvl, uint32_t ModuleId, uint32_t InstanceId, uint32_t ApiId, uint32_t ErrorId)
{
...

    {
        ...
        tB moduleAlreadyPresent_tB = false;
        for (uint32_t i_U32 = 0; i_U32 < errh_nr_activeErrorCount_U32; i_U32++)
        {
            ...
            )
            {
                ...
                errh_x_errors_vstr[i_U32].ti_globalTime = errh_x_config_str.ti_readCurrentGlobalTimeCallback_S64();
                errh_x_errors_vstr[i_U32].ti_us_timestamp = errh_x_config_str.ti_us_readSystemTimeCallback_S64();
        ...
        if (false == moduleAlreadyPresent_tB)
        {
            ...
            errh_x_errors_vstr[errh_nr_activeErrorCount_U32].ti_globalTime = errh_x_config_str.ti_readCurrentGlobalTimeCallback_S64();
            errh_x_errors_vstr[errh_nr_activeErrorCount_U32].ti_us_timestamp = errh_x_config_str.ti_us_readSystemTimeCallback_S64();
...
tB errh_canMsgCompose_100ms(uint8_t** DataPtr_MpU8)
{
    tB messagesComposed_tB = false;
...
void errh_init(tERRH_INITDATA_STR* ErrhCfg)
{
    ...
    {
        errh_x_config_str.ti_us_readSystemTimeCallback_S64 = ErrhCfg->ti_us_readSystemTimeCallback_S64;
        errh_x_config_str.ti_readCurrentGlobalTimeCallback_S64 = ErrhCfg->ti_readCurrentGlobalTimeCallback_S64;

        if (ERRH_INTERNAL_CALLBACK != ErrhCfg->reportErrorCallback)
        {
            errh_x_config_str.reportErrorCallback = ErrhCfg->reportErrorCallback;
        }
