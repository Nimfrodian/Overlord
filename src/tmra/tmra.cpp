...
void tmra_init(tTMRA_INITDATA_STR* TmraCfg)
{
    ...
    {
        errh_reportError(ERRH_NOTIF, MODULE_TMRA, 0, TMRA_API_INIT_U32, ERRH_MODULE_ALREADY_INIT);
    }
    else if (NULL == TmraCfg)
    {
        errh_reportError(ERRH_ERROR_CRITICAL, MODULE_TMRA, 0, TMRA_API_INIT_U32, ERRH_POINTER_IS_NULL);
    }
    else
    {
...
uint32_t tmra_stopTimer(tTMRA_TIMERDATA_STR* TmraTimerHandle)
{
    ...
    if (0 < tmra_ti_us_rawSysTi_U64)
    {
        ...
        if (err) {errh_reportError(ERRH_NOTIF, MODULE_TMRA, 0, TMRA_API_STOP_TIMER_U32, TMRA_ERR_CANNOT_STOP_TIMER_U32);};
        gptimer_set_raw_count(*TimerHandle, 0);
        err = gptimer_disable(TmraTimerHandle->timerHandle_str);
        if (err) {errh_reportError(ERRH_NOTIF, MODULE_TMRA, 0, TMRA_API_STOP_TIMER_U32, TMRA_ERR_CANNOT_DISABLE_TIMER_U32);};
    }
    return err;
}
...
uint32_t tmra_startTimer(tTMRA_TIMERDATA_STR* TmraTimerHandle)
{
    ...
    {
        err = 1;
        errh_reportError(ERRH_ERROR_CRITICAL, MODULE_TMRA, 0, TMRA_API_START_TIMER_U32, ERRH_POINTER_IS_NULL);
    }
    else
    {
        gptimer_handle_t* TimerHandle = &TmraTimerHandle->timerHandle_str;
        ...
        if (err) {errh_reportError(ERRH_ERROR_CRITICAL, MODULE_TMRA, 0, TMRA_API_START_TIMER_U32, TMRA_ERR_CANNOT_ENABLE_TIMER_U32);};

        err = gptimer_set_alarm_action(*TimerHandle, alarm_config);
        if (err) {errh_reportError(ERRH_NOTIF, MODULE_TMRA, 0, TMRA_API_START_TIMER_U32, TMRA_ERR_CANNOT_SET_ALARM_U32);};

        err = gptimer_start(*TimerHandle);
        if (err) {errh_reportError(ERRH_NOTIF, MODULE_TMRA, 0, TMRA_API_START_TIMER_U32, TMRA_ERR_CANNOT_START_TIMER_U32);};
    }
    return err;
}
...
uint32_t tmra_createTimer(tTMRA_TIMERDATA_STR* TmraTimerHandle)
{
    ...
    if (err)
    {
        errh_reportError(ERRH_ERROR_CRITICAL, MODULE_TMRA, err, TMRA_API_CREATE_TIMER_U32, TMRA_ERR_CANNOT_CREATE_TIMER_U32);
    }

    err = gptimer_register_event_callbacks(TmraTimerHandle->timerHandle_str, &TmraTimerHandle->cbs, nullptr);
    if (err)
    {
        errh_reportError(ERRH_ERROR_CRITICAL, MODULE_TMRA, err, TMRA_API_CREATE_TIMER_U32, TMRA_ERR_CANNOT_ADD_CB_TIMER_U32);
    }
